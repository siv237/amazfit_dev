...
## Пустой экран Settings App на телефоне (Zepp App)

**Причина: нет доступа к `zepp-os.zepp.com` (РКН/сетевые блокировки).**
Рантайм страницы настроек (`app-settings/vX`, ~710 КБ) Zepp App подгружает с
`zepp-os.zepp.com`. При блокировке/обрыве загрузки веб-вью открывается пустым —
код ошибки не показывает, приложение при этом работает. Симптом: настройки пустые
**и у вашего приложения, и у эталонных** (например, официального `todo-list`),
хотя `setting.js` в пакете есть.

**Проверено:** с VPN на телефоне настройки открываются сразу (и у приложения, и у
«дочкиного»). В контейнере та же сеть обрезает загрузку до ~15 КБ вместо ~710 КБ —
лечится локальным зеркалом (`scripts/setup-framework-mirror.sh`, `setting-service.url`).

**Что делать:**
- на телефоне включить VPN/прокси и переоткрыть карточку приложения → Настройки;
- либо поднимать `app-settings` локально (как для симулятора);
- это не баг приложения и не ошибка `setting/index.js`.
