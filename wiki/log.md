...
## [2026-09-13] GPS | `fake_data_gps.dat` НЕ управляет @zos/sensor Geolocation
Создано мини-приложение `apps/gts4/gps-test` (показ GPS + acc/gyro/compass/baro).
Проверено на GTS 4 (симулятор 2.1.2, OS 3.5):
- Compass: инъекция `fake_data_compass.dat` (вращение 20°/с) РАБОТАЕТ — за 5 c
  угол 180°→280°, статус `true`. Механизм `fake_data_*` в прошивке активен.
- Geolocation: всегда `34.052222,118.251667` (DMS 34°3'8"N 118°15'6"E):
  - не зависит от `fake_data_gps.dat`: восстановление заводского образа
    (Пекин 40.05,116.15) и инъекция файла в заводском формате дают то же значение;
  - в панели **Sensor** поля GPS нет; легаси `hmSensor` в API_LEVEL 3.0 отсутствует;
  - константа не найдена ни в `app.asar`, ни в `main.elf`/QEMU (в т.ч. f32/f64),
    ни в конфигах — вероятно, в сжатом jsfwk или эмуляции GNSS.
Итог: чтобы прокручивать GPS-точки, нужен другой канал (не fake_data_gps.dat).
Компасная имитация при этом рабочая.

## [2026-09-13] GPS | Рабочее решение: позиция через app-side провайдера
`Geolocation` в симуляторе отдаёт фиксированный дефолт (34.052222,118.251667),
не управляемый `fake_data_gps.dat`. Решение для эмулятора: приложение запрашивает
позицию у своего app-side (`GET_POSITION`), который в симуляторе читает
`http://127.0.0.1:8099/gps.json` (хост), а на реальных часах запрос не проходит —
используется штатный GNSS. Страница не знает про источник.
`scripts/dev.sh --gps/--gps-track/--gps-random` пишет `vendor/zepp-fw/gps.json`
